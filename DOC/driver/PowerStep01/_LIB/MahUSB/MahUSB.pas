unit MahUSB;                // http://stackoverflow.com/questions/7949782/detect-usb-drive-device-using-delphi
...
type
// Event Types
  TOnDevVolumeEvent = procedure(const bInserted: Boolean; const sDrive : string) of object;
//  TOnUsbChangeEvent = procedure(const bInserted: Boolean; const ADevType, ADriverName, AFriendlyName: string) of object;
  TOnUsbChangeEvent = procedure(const bInserted: Boolean; const bRemoved: Boolean; const ADevType, ADriverName, AFriendlyName: string) of object;
...
// USB Class }
  ...
  private
    ...
    procedure GetUsbInfo(const ADeviceString: string; out ADevType,ADriverDesc, AFriendlyName: string);
  ...
  public
    ...
    property OnDevVolume : TOnDevVolumeEvent read FOnDevVolumeEvent write FOnDevVolumeEvent;
  end;
...
procedure TUsbClass.GetUsbInfo(const ADeviceString: string; out ADevType, ADriverDesc, AFriendlyName: string);
var
...
// Extract the portions of the string we need for registry. eg. \\?\USB#Vid_4146&Pid_d2b5#0005050400044#{a5dcbf10- ..... -54334fb951ed}
...
// Get list of keys in \USBSTOR and enumerate each key for a key that matches our sKey2='0005050400044' NOTE : The entry we are looking for normally has '&0' appended to it eg. '0005050400044&0'
      ...
      begin
          ...
          begin
            ...
            begin
              ...
              begin
                ...
                if oReg.OpenKeyReadOnly(Format(SUBKEY2, [oKeys[i], oSubKeys[ii]])) then
                begin
        ...
        if not bFound then // Pri FTDI napravah ne najde imena, bomo takole dodali
        begin
          if sKey1= 'VID_0403&PID_6001' then begin ADriverDesc := 'USB controller'; AFriendlyName := 'FT232/FT245'; end;
          if sKey1= 'VID_0403&PID_6010' then begin ADriverDesc := 'USB controller'; AFriendlyName := 'FT2232';      end;
          if sKey1= 'VID_0403&PID_6011' then begin ADriverDesc := 'USB controller'; AFriendlyName := 'FT4232H';     end;
          if sKey1= 'VID_0403&PID_6014' then begin ADriverDesc := 'USB controller'; AFriendlyName := 'FT232H';      end;
          if sKey1= 'VID_0403&PID_601C' then begin ADriverDesc := 'USB controller'; AFriendlyName := 'FT4222H';     end;
        end;
      end;
      FreeAndNil(oKeys);
      FreeAndNil(oSubKeys);
...
begin
  ...
  begin
    ...
    if iDevType = USB_VOLUME then
      if Assigned(FOnDevVolumeEvent) then
      begin
        pVol := PDEV_BROADCAST_VOLUME(AMessage.LParam);
        FOnDevVolumeEvent((AMessage.wParam = USB_INSERTION), DriverLetter(pVol.dbcv_unitmask));
      end else
    else
    ...
    begin
      ...
      if Assigned(FOnUsbChangeEvent) then
         FOnUsbChangeEvent((AMessage.wParam = USB_INSERTION),(AMessage.wParam = USB_REMOVAL), sDevType,sDriverName,sFriendlyName);
    end;
  end;
end;
