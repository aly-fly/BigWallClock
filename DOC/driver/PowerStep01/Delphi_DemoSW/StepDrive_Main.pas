...
type
  TForm1 = class(TForm)
    ...
    procedure BtnRisiSignaleClick(Sender: TObject);
    procedure BtnStartAIClick(Sender: TObject);
    procedure BtnStopAIClick(Sender: TObject);
...
//  if (Copy(Clipboard.AsText, 1, 16 )= 'UkazZaStepDrive(') and (Copy(Clipboard.AsText, Length(Clipboard.AsText), 1 )= ')') then Clipboard.Clear;
  ...
  begin
    ...
    if meOpen(ME_OPEN_NO_FLAGS) <> ME_ERRNO_SUCCESS then ShowMessage('ME error open');
    ME4670_FindDeviceIndex;     // pogledam, ce je kartica prisotna in dodelim kartici index ...
    if ME4670_DevInd > -1 then BtnNastaviAIClick(nil);
  end else
  begin
  ...
  try
    ...
    begin
      ...
      begin
        if SN <> SN1 then
        begin
           if MessageDlg('Kontroler s serijsko štrvilko '+ SN+ ' ni prikljuèen. '+ #13#10+
...
procedure TForm1.BtnRisiSignaleClick(Sender: TObject);
var
  I: Integer;
  N: Integer;
begin
  N := Length(ME4670_AI_Config.ChanList);
  Series1.Clear;
  Series2.Clear;
  for I := 0 to ME4670_Ind_An_Buff- 1 do
  begin
    if I mod N = 0 then Series1.AddXY((I div N)* ME4670_AI_Config.ScanTime, ME4670_Buff_An_Mer[I mod 7500000]);
    if I mod N = 1 then Series2.AddXY((I div N)* ME4670_AI_Config.ScanTime, ME4670_Buff_An_Mer[I mod 7500000]);
  end;
end;

procedure TForm1.BtnStartAIClick(Sender: TObject);
var
  TbLng: Integer;
begin
  if Length(ME4670_AI_Config.ChanList) > 0 then
  begin
    TbLng := (Length(ME4670_Buff_An_Mer) div Length(ME4670_AI_Config.ChanList))+ 1;
    SetLength(ME4670_AI_EncPos, 0);
    SetLength(ME4670_AI_EncPos, TbLng);
    ME4670_Zacni_AI_Zajemanje;
  end else
    ShowMessage('Ni kanalov');
end;

procedure TForm1.BtnStopAIClick(Sender: TObject);
begin
  ME4670_Stream_Stop;
end;
...
var
  ...
  Aa: Double;
begin
  N := 100;   // vzorcev  za povprecit
  Z := 25;    // na koliko vzorcev narisat
  Series1.Clear;
  Series2.Clear;
  Series3.Clear;
  Series4.Clear;
  Aa := 0;
  for I := 0 to N- 1 do Aa := Aa+ ME4670_Buff_An_Mer[I mod 7500000]/N;
  for I := N to ME4670_Ind_An_Buff div 2 - 1 do
  begin
    Aa := Aa+ ME4670_Buff_An_Mer[I mod 7500000]/N- ME4670_Buff_An_Mer[(I- N) mod 7500000]/N;
    if I mod Z= 0 then
    begin
      Series1.AddXY(I* Z* ME4670_AI_Config.ScanTime, Aa);
    end;
  end;
end;
...
var
  ...
  Pv: Double;
begin
  N := StrToInt(EdPovp.Text);
  Series1.Clear;
  Series2.Clear;
  Pv := 0;
  for I := 0 to N- 1 do
    Pv := Pv+ ME4670_Buff_An_Mer[I mod 7500000];

  if CbVsakaDeseta.Checked then
  begin
    for I := N to ME4670_Ind_An_Buff- 1 do
    begin
      Pv := Pv+ ME4670_Buff_An_Mer[I mod 7500000]- ME4670_Buff_An_Mer[(I- N) mod 7500000];
      if I mod 10= 0 then Series1.AddXY( I* ME4670_AI_Config.ScanTime, Pv/N);
    end;
  end else
  begin
    for I := N to ME4670_Ind_An_Buff- 1 do
    begin
      Pv := Pv+ ME4670_Buff_An_Mer[I mod 7500000]- ME4670_Buff_An_Mer[(I- N) mod 7500000];
      Series1.AddXY( I* ME4670_AI_Config.ScanTime, Pv/N);
    end;
  end;
end;
...
begin
  ...
  SetLength(ME4670_AI_Config.ChanList, Length(A));

  ME4670_AI_Config.MerRange   := CmbObm.ItemIndex;
  if RgAnConnType.ItemIndex  = 0 then ME4670_AI_Config.AI_Groud_Type := ME_REF_AI_GROUND    else ME4670_AI_Config.AI_Groud_Type := ME_REF_AI_DIFFERENTIAL;
  if RgAnScanStart.ItemIndex = 0 then ME4670_AI_Config.ScanType      := ME4670_AISM_CASOVNO else ME4670_AI_Config.ScanType      := ME4670_AISM_TRIGGER;
  if RgAnAcqStart.ItemIndex  = 0 then ME4670_AI_Config.AcqStartType  := ME_TRIG_TYPE_SW     else ME4670_AI_Config.AcqStartType  := ME_TRIG_TYPE_EXT_DIGITAL;
  for I := 0 to Length(A)- 1 do ME4670_AI_Config.ChanList[I] := A[I];
  ME4670_AI_Config.StevKanalov := Length(ME4670_AI_Config.ChanList);
  ME4670_AI_Config.ChanTime      := StrToFloat(EdConvTime.Text)/1000;
  ME4670_AI_Config.ScanTime      := StrToFloat(EdScanTime.Text)+ 0.00001;
  ME4670_AI_Config.MaxSamplesNr  := StrToIntDef(EdScanSamplNr.Text, 0);  EdScanSamplNr.Text := IntToStr(ME4670_AI_Config.MaxSamplesNr);

  ME4670_Nastavi_AI_Zajemanje;

  if ME4670_AI_Config.MaxSamplesNr > 0 then
    BfLn := ME4670_AI_Config.MaxSamplesNr* ME4670_AI_Config.StevKanalov
  else
    BfLn := Length(ME4670_Buff_An_Mer);
  EdConvTime.Text := FloatToStr(ME4670_AI_Config.ChanTime*1000);
  EdScanTime.Text := FloatToStr(ME4670_AI_Config.ScanTime);
  LblSmpl.Caption := '( '+ FloatToStrF(1/ME4670_AI_Config.ScanTime, ffFixed, 32, 6)+ ' ) kHz';
  Label113.Caption := FloatToStr(ME4670_AI_Config.ScanTime* BfLn/1000)+ ' s';
end;
...
// najprej kalibracija tokmeritve
  ...
  Oa := 0;
  Ob := 0;
  for I := 0 to ME4670_Ind_An_Buff div 2 - 1 do
  begin
    Oa := Oa+ ME4670_Buff_An_Mer[(I*2+0) mod 7500000];
    Ob := Ob+ ME4670_Buff_An_Mer[(I*2+1) mod 7500000];
  end;
  Oa := Oa*2/ME4670_Ind_An_Buff;
  Ob := Ob*2/ME4670_Ind_An_Buff;
...
//  Sleep(20);
  ...
  s1 := 0;
  k := 0;
  for I := 0 to N- 1 do Aa := Aa+ ME4670_Buff_An_Mer[(I*2+0) mod 7500000]/N;
  for I := 0 to N- 1 do Ab := Ab+ ME4670_Buff_An_Mer[(I*2+1) mod 7500000]/N;
  for I := N to ME4670_Ind_An_Buff div 2 - 1 do
  begin
    Aa := Aa+ ME4670_Buff_An_Mer[(I*2+0) mod 7500000]/N- ME4670_Buff_An_Mer[((I- N)*2+0) mod 7500000]/N;
    Ab := Ab+ ME4670_Buff_An_Mer[(I*2+1) mod 7500000]/N- ME4670_Buff_An_Mer[((I- N)*2+1) mod 7500000]/N;
    if I mod Z= 0 then
    begin
      Series1.AddXY(I* ME4670_AI_Config.ScanTime, (Aa-Oa)* 10);  // 1V= 10A
      Series2.AddXY(I* ME4670_AI_Config.ScanTime, (Ab-Ob)* 10);  // 1V= 10A
      t := Sqrt(Sqr(Aa-Oa)+ Sqr(Ab-Ob))* 10;                     // 1V= 10A
      Series3.AddXY(I* ME4670_AI_Config.ScanTime, t);
      begin
        if (t > TokMeja* 0.7) and Risi then
        begin
          s2 := ArcTan2((Aa-Oa), (Ab-Ob));
          if s2- s1 <-pi then k := k+ 2*pi;
          if s2- s1 > pi then k := k- 2*pi;
          s1 := s2;
          l := (k+ s2)/2/pi;    // stevilo sinusov
          Series4.AddXY(I* ME4670_AI_Config.ScanTime, l/1000);
          if (Series4.Count > 0) and (t < TokMeja* 0.2) then Risi := False;
        end;
      end;
    end;
  end;
...
begin
  ...
  begin
    ...
    if I mod Z= 0 then
    begin
      t := Sqrt(Sqr(Aa)+ Sqr(Ab))* 10.07;                      // faktor atenuacije
      if t > 1 then
      begin
        Series1.AddXY(I* ME4670_AI_Config.ScanTime, Aa* 10.07);  // faktor atenuacije
        Series2.AddXY(I* ME4670_AI_Config.ScanTime, Ab* 10.07);  // faktor atenuacije
        Series3.AddXY(I* ME4670_AI_Config.ScanTime, t);
        s2 := ArcTan2((Aa), (Ab));
        if s2- s1 > pi then s1 := s1+ 2*pi;
        if s2- s1 < -pi then s1 := s1- 2*pi;
//        Series4.AddXY(I* ME4670_AI_Config.ScanTime, 1000/2/pi* Abs(s2-s1)/(z* ME4670_AI_Config.ScanTime));  // v 1/s
//        Series4.AddXY(I* ME4670_AI_Config.ScanTime, 10/2/pi* Abs(s2-s1)/(z* ME4670_AI_Config.ScanTime));  // v 1/s
//        Series4.AddXY(I* ME4670_AI_Config.ScanTime,t/( 1000/2/pi* Abs(s2-s1)/(z* ME4670_AI_Config.ScanTime)));
        if ( 1/2/pi* Abs(s2-s1)/(z* ME4670_AI_Config.ScanTime)) <> 0 then
          Series4.AddXY(I* ME4670_AI_Config.ScanTime, t/( 1/2/pi* Abs(s2-s1)/(z* ME4670_AI_Config.ScanTime)));
        s1 := s2;
      end;
    end;
  end;
...
//  Exit;
  ...
  Label23.Caption := IntToStr(ME4670_Ind_An_Buff);
  TimerTece := False;
end;
